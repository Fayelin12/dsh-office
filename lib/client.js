  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      if (timer && typeof timer.interval === "function" && rpc && typeof rpc.call === "function") {
          ...
          rpc.call("/office", "calendar-notice", null).then((result) => {
            ...
            if (v && Array.isArray(v.meetings) && v.meetings.length) {
              ...
              const hasSoon = v.meetings.some((m) =>
                ...
                m.start_ts >= nowMs - 3600 * 1000 && m.start_ts <= nowMs + 2 * 3600 * 1000);
        ...
        // 启动时立即拉一次（不等第一个间隔）
        meetingTimer = setTimeout(pollCalendar, 3000);
        ctx.effect(() => () => {
          disposed = true;
        ...
        function nearestMeeting(m) {
          ...
          const upcoming = m.meetings
            .filter((e) => typeof e.start_ts === "number" && isFinite(e.start_ts) &&
              e.start_ts >= nowMs - 3600 * 1000 && e.start_ts <= nowMs + 2 * 3600 * 1000)
