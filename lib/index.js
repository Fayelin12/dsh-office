...
export async function apply(ctx, _config = {}) {
  ...
  function loadFeishuConfig() {
    try {
      ...
      if (typeof sync === "string" && sync.trim() && typeof latest === "string" && latest.trim()
        ...
        return { sync, latest, mention, autoSync };
      }
  ...
  function triggerFeishuSync(cfg) {
    if (feishuSyncing) return false;
    feishuSyncing = true;
    spawnNodeScript(cfg.sync, [], FEISHU_SYNC_TIMEOUT_MS).then(({ error, stdout, stderr }) => {
  ...
  // 定时检查：autoSync 开启且距上次同步超过间隔 → 自动同步
  function checkFeishuAutoSync() {
    const cfg = loadFeishuConfig();
    const auto = feishuAutoConfig(cfg);
    if (!cfg || !auto.enabled) return;
    if (feishuSyncing) return;
    if (feishuLastSyncAgeMs() >= auto.intervalMin * 60 * 1000) {
      ctx.logger.info('[office][feishu] 自动同步触发（间隔 ' + auto.intervalMin + ' 分钟）');
      triggerFeishuSync(cfg);
    }
  }
  ...
  async function serveFeishuSync(req, res) {
    ...
    const started = triggerFeishuSync(cfg);
  ...
  // 自动同步定时检查（60s 一次，config 每次读取，改配置即时生效）
  const feishuAutoCheckTimer = setInterval(checkFeishuAutoSync, 60 * 1000);
  if (feishuAutoCheckTimer.unref) feishuAutoCheckTimer.unref();
  ...
  ctx.effect(() => {
    return () => {
      ...
      if (feishuAutoCheckTimer) clearInterval(feishuAutoCheckTimer);
    };
