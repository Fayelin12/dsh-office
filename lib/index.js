...
export async function apply(ctx, _config = {}) {
    ...
    if (connection && connection.rpc && typeof connection.rpc.handle === "function") {
      return connection.rpc.handle("/office", async (endpoint, payload) => {
        ...
        if (endpoint === "calendar-notice") {
          ...
          const upcoming = upcomingMeetings(calendarEvents, 120);
  ...
  function saveCalendarCache(since) {
    mkdir(DATA_DIR, { recursive: true })
  ...
  async function syncCalendar(cfg, opts = {}) {
    ...
    try {
      ...
      return { ok: false, error: { code: 1, message: e && e.message ? e.message : String(e) } };
    } finally {
  ...
  function upcomingMeetings(events, windowMinutes = 120) {
    const now = Date.now();
    const out = [];
    for (const e of events || []) {
      ...
      if (!ended && diffMin >= -60 && diffMin <= windowMinutes) {
  ...
  function calendarCacheStale(maxAgeMs = 10 * 60 * 1000) {
  ...
  function loadFeishuConfig() {
    try {
      ...
      if (typeof sync === "string" && sync.trim() && typeof latest === "string" && latest.trim()
        ...
        return {
          sync, latest, mention, autoSync, manualWindowDays,
    ...
    return {
      ...
      error: {
        ...
        setup: {
          ...
          docsUrl: "https://github.com/Fayelin12/dsh-office/blob/main/docs/feishu-setup.md",
          template: {
            scripts: { sync: "<ingestor.js 绝对路径>", latest: "<latest.js 绝对路径>" },
            mention: ["你的名字，可选"]
          }
  ...
  function triggerFeishuSync(cfg, opts = {}) {
    ...
    spawnNodeScript(cfg.sync, args, FEISHU_SYNC_TIMEOUT_MS).then(({ error, stdout, stderr }) => {
      ...
      if (cfg.calendar) syncCalendar(cfg).catch((e) => {
  ...
  async function serveCalendarEvents(req, res) {
    ...
    sendJson(res, { ok: true, data: {
      ...
      upcoming: upcomingMeetings(calendarEvents, 120)
    } });
  }
