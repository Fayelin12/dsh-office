...
export async function apply(ctx, _config = {}) {
    ...
    return {
      ...
      error: {
        ...
        setup: {
          ...
          template: { scripts: { sync: "<ingestor.js 绝对路径>", latest: "<latest.js 绝对路径>" } }
        }
      }
    };
  }
  ...
  async function serveFeishuStatus(req, res) {
    ...
    sendJson(res, { ok: true, data: { configured: !!cfg, syncing: feishuSyncing, lastSync: feishuLastSync } });
  }
  ...
  async function serveFeishuSync(req, res) {
    ...
    sendJson(res, { ok: true, data: { started: true } });
  }
  ...
  async function serveFeishuMessages(req, res) {
    ...
    try {
      ...
      const { error, stdout } = await spawnNodeScript(cfg.latest, ["--limit", String(limit)], FEISHU_LATEST_TIMEOUT_MS);
      let parsed = null;
      try { parsed = JSON.parse(stdout.trim()); } catch (e) { parsed = null; }
      if (parsed && parsed.ok) {
        sendJson(res, { ok: true, data: { total: parsed.total, chats: parsed.chats || [], messages: parsed.messages || [] } });
      } else {
