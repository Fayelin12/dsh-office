...
export async function apply(ctx, _config = {}) {
  ...
  const DATA_CACHE_TTL_MS = 6000; // force rebuild after 6s even without dirty flag (active sessions change)
  ...
  async function serveData(res) {
    try {
      const now = Date.now();
      // P2-2: return cached data if not dirty and within TTL
      if (!_dataDirty && _dataCache && (now - _dataLastBuilt < DATA_CACHE_TTL_MS)) {
        res.statusCode = 200;
        res.setHeader("content-type", "application/json; charset=utf-8");
        res.setHeader("cache-control", "no-store");
        res.end(_dataCache);
